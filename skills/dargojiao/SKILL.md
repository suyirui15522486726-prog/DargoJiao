...
4. 分开判断作品身份、题目和回答依据。短链解析出的平台作品 ID 是主身份；同条分享中的作者、标题和话题用于补充身份与冲突检查。无法解析作品 ID 或元数据冲突时保留为待重试。
5. 作品 ID 已确认且分享标题能表达清晰面试问题或 Topic 时，必须进入笔记生成。公开页缺少正文或字幕本身不能成为待重试理由。
6. 优先读取原链接、页面正文、字幕或同作品可信镜像来还原视频观点。拿不到这些内容时，仍按分享题目与可核验技术事实生成笔记，把错误回答明确标为“反例归纳”，并说明不是视频原话；技术事实无法核验时才保留待重试。
7. 按 `references/note-format.md` 生成面试笔记。只读检查配置的本地项目与固定 Git ref；只有知识点与真实代码直接相关时，才加入项目路径、符号和代码块，不要切换分支或修改工作区。
8. 根据 Topic 路由到最合适的 Wiki 文档；没有合适分类时才创建顶层 Docx。
9. 写入后回读，验证唯一标识仅出现一次，再计入成功数。
10. 在 Codex「已安排」任务结果中输出成功数、跳过数、待重试数、分类、异常阶段和用户动作。

## 必须保持的边界

- 不把标题相同直接当成同一视频；标题只用于冲突提醒。
- 不把分享题目与技术事实整理冒充视频逐字观点；无法核验视频原话时必须标明来源边界。
- 不因公开页缺少正文或字幕拒绝一个作品 ID 与题目均明确的正常分享。
- 不在身份、题目或技术事实不足时猜测，也不写成功标识。
