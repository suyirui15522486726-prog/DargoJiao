...
群消息应包含可点击的原始分享链接，并尽量保留作者和完整标题。短链能够解析出平台作品 ID、标题也能表达清晰面试问题时，即使公开页没有正文或字幕也会继续生成笔记；此时错误答题标为“反例归纳”，回答注明基于分享题目与可核验技术事实整理。只有封面、截图、纯标题、无法解析作品 ID、题目不明确、元数据冲突或技术事实无法核验时，才保留为“待重试”。
...
只有写入成功并回读确认后才留下成功标识。身份或题目不足、元数据冲突、技术事实无法核验、网络失败、链接错误或权限失败的链接不占序号，保留为待重试；公开页单纯缺少正文或字幕不属于失败。
...
- 作品 ID 与题目明确但没有正文或字幕时，按分享题目与可靠技术事实整理，并明确来源边界。
- 身份、题目或技术事实无法核验时不猜测、不写笔记、不写成功标识。
